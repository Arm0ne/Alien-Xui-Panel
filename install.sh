...
ALIEN_PANEL_RELEASE_BASE="https://github.com/${ALIEN_PANEL_REPO}/releases"
...
install_free_version() {
    ...
    if [[ -z "$xui_version" ]]; then
        ...
        echo ""
        echo -e "${green}-------->>>>片刻之后脚本将会自动引导安装〔Alien-Panel面板〕${plain}"
    else
        # 检查版本号中是否包含冒号
        if [[ "$xui_version" == *:* ]]; then
    ...
    install_x-ui() {
        ...
        if [ $# == 0 ]; then
            ...
            if [[ ! -n "$last_version" ]]; then
                echo -e "${red}获取 Alien-Panel 版本失败，可能是 Github API 限制，请稍后再试${plain}"
                exit 1
            fi
            echo ""
            echo -e "-----------------------------------------------------"
            echo -e "${green}--------->>获取 Alien-Panel 最新版本：${yellow}${last_version}${plain}${green}，开始安装...${plain}"
            echo -e "-----------------------------------------------------"
            echo ""
            sleep 2
            ...
            echo ""
            sleep 2
            wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(arch).tar.gz "${ALIEN_PANEL_RELEASE_BASE}/download/${last_version}/x-ui-linux-$(arch).tar.gz"
            if [[ $? -ne 0 ]]; then
                echo -e "${red}下载 Alien-Panel 失败, 请检查服务器是否可以连接至 GitHub？ ${plain}"
                exit 1
            fi
        else
            last_version=$1
            ...
            if [[ $? -ne 0 ]]; then
                echo -e "${red}下载 Alien-Panel $1 失败, 请检查此版本是否存在 ${plain}"
                exit 1
            fi
        fi
        wget -O /usr/bin/x-ui-temp "${ALIEN_PANEL_RAW_BASE}/x-ui.sh"
    ...
    ssh_forwarding
        ...
        echo -e ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
        echo ""
        echo -e "${yellow}----->>>Alien-Panel面板和Xray启动成功<<<-----${plain}"
    }
